...
from api.models import User, UserSession, Role
...
class UserService(UserBaseService):
	...
	def login(self, request, role, format=None):
		...
		if user is not None:
			if user.otp_verification is True:
				...
				self.create_update_user_session(user, token, request)
		...
		return ({"data": None,"code": status.HTTP_400_BAD_REQUEST, "message": "INVALID_|_CREDENTIALS"})
	...
	def user_authenticate(self, user_name, password, role, country_code):
		...
		try:
			...
			try:
				user = User.objects.get(phone_no=user_name, country_code=country_code)
				print(user)
				if user.check_password(password):
					return user # return user on valid credentials
			except User.DoesNotExist:
	...
	def sign_up(self, request, format=None):
		if "email" in request.data:
			try:
				...
				user = User.objects.create(email=request.data["email"].lower(), user_name = request.data["email"], profile_status=1, role=3)
				self.generate_encoded_id(user)
				self.send_otp_on_mail(request.data["email"])
				return({"data":None, "code":status.HTTP_200_OK, "message":OTP_SENT})
		else:
			try:
				...
				user = User.objects.create(country_code = request.data["country_code"], phone_no=request.data["phone_no"], user_name=request.data["phone_no"], profile_status=1, role=3)
				self.generate_encoded_id(user)
				self.send_mobile_otp(user)
				return({"data":None, "code":status.HTTP_200_OK, "message":OTP_SENT})
	...
	def verify_otp(self, request, format=None):
		...
		if user.otp_verification is False:
	...
	def get_user_details_by_token(self, request, format=None):
		try:
			...
			details_obj =  User.objects.get(pk=request.user.id)
			serializer= UserUpdateDetialsSerializer(details_obj)
	...
	def update_profile_detials(self, request, format=None):
		"""
		Updates detail
		"""
		data = request.data
